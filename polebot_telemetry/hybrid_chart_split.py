#!/usr/bin/env python3
"""
hybrid_chart_split_v2.py
Versi perbaikan: fix NaN metrik, bar chart 3-bar, anotasi zoom bersih,
kondisi klasifikasi scatter, dan jawab pertanyaan dospem.
"""
...
# Warna — konsisten dengan hybrid_switching_chart.png
C_STATIC  = '#003087'   # biru navy  — STATIS / ARIMA
C_DYNAMIC = '#CC0000'   # merah gelap— DINAMIS / XGBoost
C_HYBRID  = '#1A7A4A'   # hijau tua  — prediksi hybrid
C_BG_ST   = '#C5D9F2'   # biru abu   — zona statis 
C_BG_DY   = '#F2C5C5'   # merah abu  — zona dinamis
...
def plot_threshold_analysis(df, is_static):
    ...
    # Label zona
    ax1.text(SPEED_THRESHOLD*0.3, ACCEL_THRESHOLD*0.3,
             'STATIC ZONE\n(ARIMA active)',
    ...
    ax1.text(SPEED_THRESHOLD*1.6, ACCEL_THRESHOLD*3,
             'DYNAMIC ZONE\n(XGBoost active)',
    ...
    ax1.set_xlim(-0.02, min(vmax, 1.0))
    ax1.set_ylim(-0.01, min(amax, 3.0))
    ...
    ax2.legend(fontsize=9, framealpha=0.9, edgecolor='#BBBBBB')
