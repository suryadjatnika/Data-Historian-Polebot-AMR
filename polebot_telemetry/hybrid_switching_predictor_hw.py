...
# Parameter filter kondisi
ACCEL_THRESHOLD = 0.10   # m/s²  batas maksimum akselerasi untuk statis
SPEED_THRESHOLD = 0.18   # m/s   batas maksimum kecepatan untuk statis
...
# variabel target yang ditampilkan di chart
TARGETS = [
...
# Warna chart
COLOR_STATIC_LINE  = '#003087'   # data aktual segmen STATIS
COLOR_DYNAMIC_LINE = '#CC0000'   # data aktual segmen DINAMIS
COLOR_HYBRID_PRED  = '#00A651'   # prediksi
COLOR_STATIC_BG    = '#D6E8FF'   # background region statis
COLOR_DYNAMIC_BG   = '#FFD6D6'   # background region dinamis
...
# KLASIFIKASI KONDISI PER TIMESTEP
...
# MODEL XGBOOST FIT DAN PREDIKSI
...
# GABUNGKAN PREDIKSI BERDASARKAN KONDISI
...
def plot_switching_chart(df: pd.DataFrame,
    ...
    for row, (field, title, unit) in enumerate(panel_info):
        ...
        # Plot data aktual berwarna per segmen
        i = 0
        while i < len(idx):
            j = i + 1
...
def main():
    print("\n" + "═"*60)
    print("  HYBRID SWITCHING PREDICTOR")
    print("  Condition-Based Temporal Switching")
    print("  Skenario Mixed Switching Demo")
    print("═"*60)
