...
OUTPUT_DIR      = os.path.expanduser("~/arima_results_demo")
...
'''
            ...
            if not df_dyn.empty:
                ...
                mask_static = (
                    df_dyn['odom_accel'].abs() < 0.10
                ) & (
                    df_dyn['odom_v_linear'].abs() < 0.18
                )
...
def adf_test(series: pd.Series, label: str) -> dict:
    ...
    print(f"  Stasioner     : {'Ya ✅ (d=0)' if is_stationary else 'Tidak (perlu differencing, d≥1)'}")
...
def plot_results(series: pd.Series, train: pd.Series,
    ...
    ax1.plot(series.index, series.values,
             color=COLOR_HIST, linewidth=1.3, alpha=0.85,
    ...
    ax4.plot(train.index, train.values,
             color=COLOR_TRAIN, linewidth=1.0, alpha=0.7,
             label='Training data')
    ax4.plot(test.index, test.values,
             color=COLOR_ACTUAL, linewidth=1.8,
             label='Actual (test)')
...
def save_summary(results: list, output_dir: str):
    ...
    # JSON
    json_path = os.path.join(output_dir, 'arima_hw_summary.json')
    with open(json_path, 'w') as f:
        json.dump(results, f, indent=2, default=str)

    txt_path = os.path.join(output_dir, 'arima_hw_summary.txt')
...
def main():
    ...
    for config in TARGET_VARIABLES:
        ...
        if series is None or len(series) < 50:
            print(f"Data tidak cukup untuk {field}, dilewati.")
            continue
        ...
        if field == 'batt_soc_percent' and d == 0:
            print("Override d=0→1 untuk SOC (non-stasioner fisik, ADF mungkin keliru)")
            d = 1
        ...
        # 6. Plot
        plot_path = os.path.join(OUTPUT_DIR, f'arima_hw_{field}.png')
        plot_results(
        ...
        forecast_csv = os.path.join(OUTPUT_DIR, f'forecast_hw_{field}.csv')
    ...
    if summary_results:
        ...
        print("\n Tidak ada variabel yang berhasil diproses.")
