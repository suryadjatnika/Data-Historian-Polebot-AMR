...
OUTPUT_DIR  = os.path.expanduser("~/polebot_xgboost_results")
...
'''
    ...
    try:
        ...
        if 'odom_accel' in df.columns and 'odom_v_linear' in df.columns:
            df['is_dynamic'] = (
                (df['odom_accel'].abs() >= 0.05) |
                (df['odom_v_linear'].abs() >= 0.5)
            )
...
def train_xgboost(df: pd.DataFrame, config: dict) -> dict:
    ...
    # XGBoost untuk SOC hanya dilatih pada kondisi:
    # akselerasi tinggi ATAU kecepatan tinggi
    ...
    # Kalau actual=0 dan pred≈0, prediksi benar tapi sMAPE → 200%
    # (artefak matematis, bukan error model)
    ...
    if _mask_smape.sum() > 10:
        ...
        smape = 0.0  # tidak cukup data non-nol
...
def plot_results(result: dict, config: dict, output_path: str):
    ...
    ax3.plot(train_series.index, train_series.values,
             color=COLOR_TRAIN, linewidth=0.6, alpha=0.65,
             label='Training data')
    ax3.plot(test_series.index, test_series.values,
             color=COLOR_ACTUAL, linewidth=0.9,
             label='Actual (test)')
...
def save_summary(results: list, output_dir: str):
    ...
    txt_path = os.path.join(output_dir, 'xgboost_summary.txt')
    ...
    json_path = os.path.join(output_dir, 'xgboost_summary.json')
...
def main():
    ...
    for config in TARGET_VARIABLES:
        ...
        if field not in df.columns:
            print(f"\n⚠️  Field '{field}' tidak ditemukan, dilewati.")
            continue
        ...
        # Plot
        plot_path = os.path.join(OUTPUT_DIR, f'xgboost_{field}.png')
        plot_results(result, config, plot_path)

        # Simpan forecast CSV
        csv_path = os.path.join(OUTPUT_DIR, f'xgboost_forecast_{field}.csv')
