...
OUTPUT_DIR = os.path.expanduser("~/polebot_comparison_results")
ARIMA_FILE = os.path.expanduser("~/polebot_arima_results/arima_summary.json")
XGB_FILE   = os.path.expanduser("~/polebot_xgboost_results/xgboost_summary.json")

# Warna per metode — standar akademis
C_ARIMA = '#2166AC'   # biru tua
C_XGB   = '#1A9641'   # hijau tua
...
# Warna tema akademis (putih)
AX_BG   = 'white'
AX_TEXT = '#222222'
AX_GRID = '#CCCCCC'
FIG_BG  = 'white'

# Setup global style jurnal
...
def load_results():
    def read_json(path, name):
        try:
            ...
            print(f"  ⚠️  {name}: file tidak ditemukan ({path})")
            return {}
...
def plot_bar_comparison(arima, xgb):
    """Bar chart komparasi ARIMA vs XGBoost — gaya jurnal akademis."""
    ...
    fig.suptitle(
        ...
        'Predictive Analytics System — Polebot AMR · Polman Bandung 2025/2026\n'
...
def plot_winner_table(arima, xgb):
    """Tabel ringkas pemenang per variabel per metrik — gaya jurnal akademis."""
    fig, ax = plt.subplots(figsize=(10, 3.5))
    fig.patch.set_facecolor('white')
    ax.set_facecolor('white')

    # Warna sel: biru muda untuk ARIMA menang, hijau muda untuk XGBoost menang
    method_fill = {0: '#CCEEFF', 1: '#CCEECC'}   # ARIMA=biru muda, XGB=hijau muda
    ...
    fig.suptitle(
        ...
        'Polebot AMR · Polman Bandung 2025/2026',
...
def plot_hybrid_justification(arima, xgb):
    ...
    C_SMAPE = '#D7191C'   # merah — garis sMAPE
...
def save_txt(arima, xgb):
    ...
    with open(path, 'w') as f:
        f.write("=" * 65 + "\n")
        f.write("KOMPARASI ARIMA vs XGBoost — Polebot AMR\n")
        ...
        f.write("=" * 65 + "\n")
        f.write("KESIMPULAN BERDASARKAN DATA AKTUAL\n")
        f.write("─" * 65 + "\n")
        for var in VARIABLES:
            field   = var['field']
            a_mae   = get_val(arima, field, 'mae')
...
def plot_forecast_overlay(arima: dict, xgb: dict):
    """
    Grafik beriringan ARIMA vs XGBoost untuk 3 variabel sekaligus.
    Setiap panel menampilkan:
      - Data historis (abu, dari InfluxDB)
      - Forecast ARIMA 60 detik ke depan (biru putus-putus)
      - Forecast XGBoost 60 detik ke depan (hijau putus-putus)
      - Anotasi MAE dan sMAPE kedua model

    Membaca CSV forecast dari folder hasil masing-masing predictor.
    """
    import pandas as pd
    ...
    ARIMA_DIR = os.path.expanduser("~/polebot_arima_results")
    XGB_DIR   = os.path.expanduser("~/polebot_xgboost_results")

    COLOR_HIST    = '#888888'   # abu    — data historis
    COLOR_ARIMA_F = '#2166AC'   # biru   — forecast ARIMA
    COLOR_XGB_F   = '#1A9641'   # hijau  — forecast XGBoost
    COLOR_CI      = '#AACCEE'   # biru muda — CI ARIMA
...
'''
        ...
        try:
            ...
            print(f"    ⚠️  Gagal ambil historis {field}: {e}")
        ...
        if os.path.exists(arima_csv):
            try:
                ...
                print(f"    ⚠️  Gagal baca ARIMA forecast {field}: {e}")

        # ── 3. Baca forecast XGBoost ──────────────────────────────
        xgb_csv = os.path.join(XGB_DIR, f'forecast_{field}.csv')
        xgb_drawn = False
        if os.path.exists(xgb_csv):
            try:
                ...
                print(f"    ⚠️  Gagal baca XGBoost forecast {field}: {e}")
    ...
    fig.suptitle(
        ...
        'Polebot AMR · Politeknik Manufaktur Bandung · 2025/2026',
...
def main():
    ...
    print("  Polebot AMR · Polman Bandung 2025/2026")
    print("=" * 60)
